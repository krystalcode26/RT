...
HTTP Methods:
- GET: retrieve data
- POST: create new data (not Idempotent: everytime add one more row) 
- PUT: update/replace whole data
- PATCH: only update partial resource -- several columns (not Idempotent)
- DELETE: remove data
Idempotent: an operation if calling it multiple times has same effect as calling it once.

How to make POST idempotent? 
- attach a unique order ID 
- make sure that the same operations is going to be filtered or rate limited either in the front-end and back end. 
- In database before we insert, should always check whether the same record has already been inside the order table. 
If that's the case, do not allow it to insert so we can manually Implement.

Status code (five levels from 1XX-5XX)
- 200: the request has been received processed successfully by the back end, and the response has already been sending back. 
- 3xx: the request has been redirect sometimes. 
- 4xx: user's invalid input,so cannot get proper respond from server. But sometimes server is down, the API Gateway is downgraded the service, the server for that service is not running.
        Ex: 404 for not found
- 5xx: server error
  
HTTP status: industrial level convention that any of the web application that's building up based on the HTTP request and response is following those rules for HTTP status.
Ex: GET /doc/test.html   HTTP/1.1 -- Request line

Request Headers:
- Host: www.test101.com
- Accept: image/gif, image/jpeg, */*
- Accept-Language: en-us
- Accept-Encoding: gzip, deflate --> http request -> %2XWJ (encoded)
...
Adding more power to the existing server — more CPU cores, more RAM, faster storage. 
It's simple but has a physical limit slot: you can only make one machine so powerful, and it's often expensive.

Horizontal Scaling (Scale Out): 
Adding more servers and spreading the load across them. Instead of making one machine bigger, 
you add five smaller ones. Ex: Netflix handle millions of users — by running hundreds or thousands of servers in parallel. 
Horizontal scaling is more flexible and fault-tolerant: if one server fails, the others keep working.

Monolithic applications can do both vertical scaling and horizontal scaling, but they are usually better suited for vertical scaling.
Vertical scaling is easier because you simply increase the CPU and memory of the server. 
...
and another for payments. Each service has its own codebase and database, and they talk to each other over the network (usually 
via HTTP APIs or message queues). This makes it easier to deploy, scale, and maintain each part independently.

Microfrontend: 
A microfrontend applies the same idea to the user interface. Instead of one big frontend application, different teams own different parts of the UI
...
These databases are great for data with clear structure and relationships, like banking transactions or HR records. 
Ex: MySQL, PostgreSQL, Oracle.

Non-Relational Database (NoSQL Database)
NoSQL databases store data in more flexible formats — as documents (like JSON), key-value pairs, graphs, or columns. 
They don't require a fixed table structure, so they can handle unstructured or rapidly changing data more easily. 
They're also often easier to scale horizontally. 
...
This is useful for tasks that take a long time (like sending emails or processing video uploads) 
...
Monitor: Monitoring is about watching your system in real time and getting alerted when something looks wrong 
Ex: CPU usage spiking above 90%, response times getting slow, or error rates climbing. Unlike logs (which tell you what happened), 
...
• Packaging your code (into a Docker container or a deployable artifact)
• Choosing a compute service (like AWS EC2 for virtual machines, AWS ECS/EKS for containers, or AWS Lambda for serverless functions)
• Setting up networking (VPC, subnets, security groups)
• Connecting to managed databases (like RDS for SQL or DynamoDB for NoSQL)
...
Ex: Role-Based Access Control (RBAC) and claims-based tokens like JWT.
...
•  End-to-End Tests: Simulate a real user flow through the entire application
